...
- `zh-tw/*`：Rally Technology 官方繁中對外文案層，依現行 runtime i18n 與專案命名決策回寫。
...
5. 若任務是回寫 public copy，先對齊目前 `i18n/locales/*` 與 `docs/project/02`、`docs/project/03` 的正式命名與 CTA，再寫回 `en / zh-tw`。
...
- 破圖 / 缺圖造成的 placeholder 文字，例如 `About Image`、`Icon Image`、`Why Us`、`Shape Image`、`Image`，只記錄在 anomaly，不納入三層 block content。

## 維護邊界

- `source` 不改寫、不摘要、不美化，只記錄參考站目前可見文字。
- `en` 與 `zh-tw` 必須維持和 `source` 相同的 block 名稱與順序。
- `en / zh-tw` 不回頭跟著 `source` 的舊 nav、舊 footer 或舊服務命名走；以現行官方多語系與 `project/02`、`project/03` 的決策為準。
- `zh-tw` 的 H1 採「中文主標 + 英文副標」。
- 跨頁異常統一查閱 [source-anomalies.md](./source-anomalies.md)；頁面可見殘留內容則直接留在各頁 block 內。
- [source-anomalies.md](./source-anomalies.md) 只負責 reference 判讀，不承擔 public copy 決策。
