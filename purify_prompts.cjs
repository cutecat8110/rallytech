...
const avoidItemsToAdd = [
  "embedded text", "slogans", "logos", "buttons", "CTA elements", "website UI overlays", "graphic design artifacts", "watermarks", "HUD decorations on screens", "fake headlines"
];

const goalSentence = "THE IMAGE MUST BE COMPLETELY FREE OF ANY EMBEDDED TEXT, SLOGANS, LOGOS, BUTTONS, OR WEBSITE UI ELEMENTS.";
const badWords = ["headline", "CTA", "button", "typography", "slogan", "text overlay"];
...
files.forEach(file => {
  ...
  if (isJson) {
    ...
    if (data.asset_type) {
      ...
      if (lowerType.includes("hero") || lowerType.includes("background") || lowerType.includes("homepage")) {
    ...
    const cleanValue = (val, key) => {
      ...
      if (typeof val === 'string') {
        ...
        if (val.includes(goalSentence)) {
          ...
          return (cleaned + (cleaned.endsWith('.') ? " " : ". ") + goalSentence).trim().replace("..", ".");
        }
        return cleanString(val);
      } else if (Array.isArray(val)) {
        return val.filter(item => {
          if (typeof item === 'string') {
            return !badWords.some(word => item.toLowerCase().includes(word.toLowerCase()));
          }
          return true;
        });
    ...
    const cleanString = (str) => {
      ...
      for (let i = 0; i < sentences.length; i += 2) {
        ...
        if (s && badWords.some(word => s.toLowerCase().includes(word.toLowerCase()))) {
          // skip
        } else {
          result.push(s + punc);
        }
    ...
    const replaceSpatial = (obj) => {
      for (let k in obj) {
        if (typeof obj[k] === 'string') {
          obj[k] = obj[k].replace(/for text overlay/gi, "as empty architectural negative space");
        } else if (Array.isArray(obj[k])) {
          obj[k] = obj[k].map(item => typeof item === 'string' ? item.replace(/for text overlay/gi, "as empty architectural negative space") : item);
    ...
    avoidItemsToAdd.forEach(item => {
      if (!data.avoid.some(existing => existing.toLowerCase() === item.toLowerCase())) {
    ...
    const assetRegex = /(homepage hero|service detail page hero|hero background|homepage .* background image|.* hero image)/gi;
    ...
    content = content.replace(/for text overlay/gi, "as empty architectural negative space");
    ...
    for (let line of lines) {
      ...
      if (inAvoid) {
        ...
        const hasBadWord = badWords.some(word => lowerLine.includes(word.toLowerCase()));
        if (hasBadWord) {
          // skip
        } else {
          newLines.push(line);
